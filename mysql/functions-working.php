    ...
    if ($shift_pattern == '2 shifts') {
        ...
        if ($datetime < $date." ".$shifts['shift1']['start']) {
            ...
            if ($datetime >= $date." ".$shifts['shift1']['start']
                ...
                if ($datetime >= $date." ".$shifts['shift2']['start']
                    && $datetime < $date." ".$shifts['shift3']['start']
                ) {
                    $shift['shift'] = "shift2";
                    $shift['date'] = $date;
...
function read_users()
{
    ...
    foreach ($users as $user) {
        ...
        if (!empty($user['last_login'])) {
            echo '<td>'.date('d/m/Y H:i:s', strtotime($user['last_login']))
                .'</td>';
        } else {
            echo '<td></td>';
        }
    ...
    foreach ($part_barcodes as $index => $part_barcode) {
        ...
        if ($part) {
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                if (($row['area'] == 'System Fill') && ($part['sf'] == 0)
...
function read_container_devan($post_data)
{
    ...
    if (count($container_devan) > 0) {
        foreach ($container_devan as $index => $row) {
            ...
            echo '<th style="font-weight: bold;">'.date('d-M D',
                    strtotime($date)).'</th>';
...
function read_container_devan_member_screen($post_data)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if (strpos(strtolower($devan_plan), "devan") !== false
        ...
        if (!strpos(strtolower($devan_plan), "devan")
...
function read_area_lane_status($post_data, $direction = null)
{
    ...
    foreach ($areas as $area => $lanes) {
        ...
        foreach ($lanes as $lane) {
            ...
            // else
            array_push($stock_available_lanes,
    ...
    $oldest_scanned_lanes_html
        = get_oldest_lanes_scanned($stock_available_lanes, $post_data,
        $direction, $area_have_most_stock, $lanes_have_most_stock);
...
function get_box_data($post_data)
{
    ...
    if ($area == 'Free Location') {
        ...
        $e = $s - $height;
        for (
            $i = $box_index * $height + 1; $i <= ($box_index + 1) * $height;
            $i++
        ) {
...
function read_kanban_box($post_data)
{
    ...
    {
        $query
            = "SELECT DISTINCT pk.dolly FROM part_to_kanban pk INNER JOIN {$tblConveyancePicks} cp ON cp.kanban_date='{$pick_date}' AND cp.kanban = pk.kanban AND LENGTH(pk.dolly) > 0 AND cp.cycle={$cur} ORDER BY pk.dolly";
        $result = $db->query($query);
    ...
    if ($max_cycle) {
        //Get pick status
        $query
            = "SELECT * FROM {$tblConveyancePicks} WHERE kanban_date = '{$pick_date}' AND cycle={$cur}";
        $result = $db->query($query);
        $total = mysqli_num_rows($result);
        //Get picked kanban
        if ($post_data['status'] == 'delivery') {
            $query
                = "SELECT * FROM {$tblConveyancePicks} WHERE is_delivered = 1 AND kanban_date = '{$pick_date}' AND cycle={$cur}";
        } else {
            $query
                = "SELECT * FROM {$tblConveyancePicks} WHERE is_completed = 1 AND kanban_date = '{$pick_date}' AND cycle={$cur}";
        }
        $result = $db->query($query);
        ...
        if ($kanban_id == 0) {
            if ($status == 'pick') {
                ...
                if ($zone != 'All') //Help or Other filters
                {
                    ...
                    {
                        $query
                            = "SELECT cp.* FROM {$tblConveyancePicks} as cp INNER JOIN part_to_kanban as pk ON cp.kanban = pk.kanban AND cp.kanban_date = '{$pick_date}' AND cp.cycle='{$post_data['cycle']}' AND cp.dolly = '{$zone}' ORDER BY  cp.is_help ASC, cp.is_completed ASC, cp.is_delivered ASC, cp.imported_at ASC, cp.pick_seq ASC, cp.kanban ASC";
                    } else //Help filter
                    {
                        $query
                            = "SELECT cp.* FROM {$tblConveyancePicks} as cp INNER JOIN part_to_kanban as pk ON cp.kanban = pk.kanban AND cp.kanban_date = '{$pick_date}' AND cp.cycle='{$post_data['cycle']}' AND cp.is_help = 1 ORDER BY  cp.is_help ASC, cp.is_completed ASC, cp.is_delivered ASC, cp.imported_at ASC, cp.pick_seq ASC, cp.kanban ASC";
                    }
                } else // All filter
                {
                    $query
                        = "SELECT * FROM {$tblConveyancePicks} WHERE kanban_date = '{$pick_date}' AND cycle=1 ORDER BY  `is_help` ASC, `is_completed` ASC, `imported_at` ASC, `pick_seq` ASC, `kanban` ASC";
                }
                // $query = "SELECT * FROM {$tblConveyancePicks} WHERE kanban_date = '{$pick_date}' AND cycle={$post_data['cycle']} ORDER BY `is_help` ASC, `is_completed` ASC, `imported_at` ASC, `kanban` ASC LIMIT 1";
        ...
        $dolly_colors = array_combine(array_column($dolly_colors, "name"),
            array_column($dolly_colors, "color"));
        if ($num_rows > 0) {
            $res = mysqli_fetch_all($result, MYSQLI_ASSOC);
            if ($status == 'delivery') {
                $arr = [
                    "SPS2", "ASSY1", "ASSY2", "SB1", "SB2", "ASSY3", "HS1",
                    "KARAKURI", "CH2", "SPS3", "SPS4"
                ];
                usort($res, function($a, $b) use ($arr) {
                    $ind1 = 1111111;
                    $ind2 = 1111111;
                    foreach ($arr as $index => $each_arr) {
                        if (strpos($a['delivery_address'], $each_arr)
                            !== false
                        ) {
                            $ind1 = $index;
                        }
                        if (strpos($b['delivery_address'], $each_arr)
                            !== false
                        ) {
                            $ind2 = $index;
                        }
                    ...
                    if ($ind1 > $ind2) {
                        ...
                        return strcmp($a['delivery_address'],
                            $b['delivery_address']);
                    }
                });
            }
...
function get_pick_address_by_kanban_and_delivery($kanban, $delivery_address)
{
    ...
    $query
        = "SELECT * FROM {$tblPart2Kanban} WHERE kanban LIKE '{$kanban}%' AND REPLACE(REPLACE(delivery_address, '-', ''), '.' ,'') = '{$d_address}'";
    $result = $db->query($query);
    $k = mysqli_fetch_object($result);
    if ($k) {
        // var_dump($k);
...
function read_pick_list($post_data)
{
    ...
    // else
    {
        if ($zone != '0') //Help or Other filters
        {
            ...
            {
                $query
                    = "SELECT cp.* FROM {$tblConveyancePicks} as cp INNER JOIN part_to_kanban as pk ON cp.kanban = pk.kanban AND cp.kanban_date = '{$pick_date}' AND cp.cycle='{$post_data['cycle']}' AND cp.dolly = '{$zone}' ORDER BY cp.address ASC, cp.is_help ASC, cp.is_completed ASC, cp.is_delivered ASC, cp.imported_at ASC, cp.pick_seq ASC, cp.kanban ASC";
            } else //Help filter
            {
                $query
                    = "SELECT cp.* FROM {$tblConveyancePicks} as cp INNER JOIN part_to_kanban as pk ON cp.kanban = pk.kanban AND cp.kanban_date = '{$pick_date}' AND cp.cycle='{$post_data['cycle']}' AND cp.is_help = 1 ORDER BY cp.address ASC, cp.is_help ASC, cp.is_completed ASC, cp.is_delivered ASC, cp.imported_at ASC, cp.pick_seq ASC, cp.kanban ASC";
            }
        } else // All filter
        ...
        {
            $query
                = "SELECT * FROM {$tblConveyancePicks} WHERE kanban_date = '{$pick_date}' AND cycle='{$post_data['cycle']}' ORDER BY `address` ASC, `is_help` ASC, `is_completed` ASC, `imported_at` ASC, `pick_seq` ASC, `kanban` ASC";
        }
    }
    ...
    if ($status == 'delivery') {
        $arr = [
            "SPS2", "ASSY1", "ASSY2", "SB1", "SB2", "ASSY3", "HS1", "KARAKURI",
            "CH2", "SPS3", "SPS4"
        ];
        usort($res, function($a, $b) use ($arr) {
            $ind1 = 1111111;
...
function search_kanban($post_data)
{
    ...
    if ($kanban) {
        $query
            = "SELECT * FROM {$tblConveyancePicks} WHERE kanban = '{$kanban}' AND cycle = {$cycle}";
        $result = $db->query($query);
        ...
        if ($num_rows > 0) {
            ...
            if (!$row->is_completed) {
                $query
                    = "UPDATE {$tblConveyancePicks} SET is_completed = 1 WHERE id = {$kanban_id}";
                $res = $db->query($query);
                $row->is_completed = 1;
                $row->res = $res;
            }
...
function get_kanban_id_by_name($post_data)
{
    ...
    // $status = $post_data['status'];
    $query
        = "SELECT id FROM {$tblConveyancePicks} WHERE kanban_date = '{$pick_date}' AND kanban = '{$kanban}' AND cycle = {$cycle} limit 1";
    $result = $db->query($query);
...
function read_excel($post_data)
{
    ...
    $dayQuery = "SELECT * FROM {$tblExcelPick} 
        ...
        AND shift = 1 
        AND Stocking_Date = '{$post_data['date']}';";
